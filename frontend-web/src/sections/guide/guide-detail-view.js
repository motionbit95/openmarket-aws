...
import { getGuideAttachments } from "src/actions/guide";

export function GuideDetailView({ currentGuide, loading, error }) {
  ...
  useEffect(() => {
    async function fetchAttachments() {
      ...
      try {
        const files = await getGuideAttachments(currentGuide.id);
  ...
  if (!currentGuide) return <Typography>문의가 존재하지 않습니다.</Typography>;
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: "auto" }}>
      ...
      >
        <div>작성일: {fDateTime(currentGuide.created_at)}</div>
        {currentGuide.updated_at && (
          <div>수정일: {fDateTime(currentGuide.updated_at)}</div>
        )}
      </Stack>

      <Stack direction={"row"} gap={1}>
        <Chip
          label={currentGuide.type}
          color="primary"
          size="small"
          variant="outlined"
        />
      ...
      >
        <Markdown children={currentGuide.content} />
      </Box>
    </Box>
  );
}
