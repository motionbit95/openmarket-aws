...
import { deleteFAQ, getAllFAQs } from "src/actions/faq";

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: "type", label: "대상", width: 100 }, // seller → "판매자", user → "사용자"
...
export function FAQListView() {
  ...
  const fetchFAQ = async () => {
    try {
      ...
      console.log("FAQ 응답 데이터:", res);

      // API 응답 구조에 따라 데이터 추출
      const faqData = Array.isArray(res) ? res : res?.data || res?.faqs || [];

      console.log("처리된 FAQ 데이터:", faqData);

      // 첫 번째 항목의 구조 확인
      if (faqData.length > 0) {
        console.log("첫 번째 FAQ 항목:", faqData[0]);
        console.log("target_audience 필드:", faqData[0].target_audience);
      }
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await deleteFAQ(id); // ✅ 서버에 먼저 삭제 요청
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      toast.success("선택한 FAQ가 삭제되었습니다.");
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      content={
        <>
          선택한 <strong>{table.selected.length}</strong>개의 FAQ를
          삭제하시겠습니까?
        </>
      }
      action={
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (FAQ) =>
        FAQ.senderName.toLowerCase().includes(name.toLowerCase()) ||
        FAQ.senderName.toLowerCase().includes(name.toLowerCase()) ||
        FAQ.title.toLowerCase().includes(name.toLowerCase())
    );
  }
