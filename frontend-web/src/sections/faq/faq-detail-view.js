...
export function FAQDetailView({ currentFAQ, loading, error }) {
  const showAttachments = useBoolean(true);
  const [answerContent, setAnswerContent] = useState(currentFAQ?.answer || "");
  const [answerSubmitting, setAnswerSubmitting] = React.useState(false);
  const [attachments, setAttachments] = useState([]);

  if (loading) return <Typography>로딩 중...</Typography>;
  if (error) return <Typography color="error">{error}</Typography>;
  if (!currentFAQ) return <Typography>문의가 존재하지 않습니다.</Typography>;
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: "auto" }}>
      ...
      >
        <div>작성일: {fDateTime(currentFAQ.created_at)}</div>
        {currentFAQ.updated_at && (
          <div>수정일: {fDateTime(currentFAQ.updated_at)}</div>
        )}
      </Stack>

      <Stack direction={"row"} gap={1}>
        <Chip
          label={currentFAQ.type}
          color="primary"
          size="small"
          variant="outlined"
        />
      ...
      >
        <Markdown children={currentFAQ.content} />
      </Box>
    </Box>
  );
}
