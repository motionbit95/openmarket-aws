...
const TABLE_HEAD = [
  ...
  { id: "email", label: "이메일", width: 100 },
  { id: "phone", label: "전화번호", width: 220 },
  { id: "review_cnt", label: "리뷰수", width: 180 },
  { id: "mileage", label: "마일리지", width: 180 },
  { id: "", width: 88 },
];
...
export function UserListView() {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await deleteUser(id); // ✅ 서버에 먼저 삭제 요청
  ...
  const fetchUsers = async () => {
    try {
      const res = await getUsers({ search: filters.state.name });
      setTableData(res);
    } catch (err) {
      console.error("유저 불러오기 실패", err);
    }
