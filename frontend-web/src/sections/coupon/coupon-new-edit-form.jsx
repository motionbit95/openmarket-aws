...
function getDefaultValues(currentCoupon) {
  ...
  console.log("쿠폰 데이터:", currentCoupon);

  // coupon_type 변환: API는 대문자, 폼은 소문자
  const couponTypeMap = {
    ALL: "all",
    USER: "user",
    SELLER_FIRST: "seller_first",
    SELLER_REPEAT: "seller_repeat",
    SELLER_MESSAGE: "seller_message",
  };

  // validity_type 변환: API는 대문자, 폼은 소문자
  const validityTypeMap = {
    FIXED_DATE: "fixed",
    FROM_ISSUE: "from_issue",
  };

  // discount_mode 변환: API는 소문자 가능, 폼도 소문자
  let inferredDiscountMode = "";
  if (currentCoupon?.discount_mode) {
    inferredDiscountMode = currentCoupon.discount_mode.toLowerCase();
  } else if (currentCoupon?.discount_max != null) {
    // discount_max가 있으면 퍼센트 할인일 가능성이 높음
    inferredDiscountMode = "percent";
  } else if (currentCoupon?.discount_amount != null) {
    // 기본값은 정액 할인
    inferredDiscountMode = "amount";
  }

  // issued_by 변환: API는 대문자, 폼은 소문자
  const issuedByMap = {
    ADMIN: "admin",
    PARTNER: "partner",
  };
  ...
  return {
    ...
    content: currentCoupon?.content || "",
    coupon_type:
      currentCoupon?.coupon_type != null
        ? couponTypeMap[currentCoupon.coupon_type] ||
          String(currentCoupon.coupon_type).toLowerCase()
        : "",
    discount_mode: inferredDiscountMode,
    discount_amount:
    ...
    total_count:
      currentCoupon?.total_count != null
        ...
        : "",
    issuer_type: currentCoupon?.issued_by
      ? issuedByMap[currentCoupon.issued_by] ||
        currentCoupon.issued_by.toLowerCase()
    ...
    end_date: currentCoupon?.end_date
      ...
      : null,
    validity_type: currentCoupon?.validity_type
      ? validityTypeMap[currentCoupon.validity_type] ||
        currentCoupon.validity_type.toLowerCase()
      : "fixed",
...
export function CouponNewEditForm({ currentCoupon, onClose, onSubmitted }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // 폼 값을 백엔드 API 형식으로 변환
      const couponTypeToAPI = {
        all: "ALL",
        user: "USER",
        seller_first: "SELLER_FIRST",
        seller_repeat: "SELLER_REPEAT",
      ...
      const parsedData = {
        ...
        coupon_type:
          couponTypeToAPI[data.coupon_type] || data.coupon_type.toUpperCase(),
        ...
        issued_by:
          issuedByToAPI[data.issuer_type] || data.issuer_type.toUpperCase(),
        validity_type:
          validityTypeToAPI[data.validity_type] ||
          data.validity_type.toUpperCase(),
        validity_days:
      ...
      console.log("쿠폰 저장 성공, 콜백 호출 중...");

      // 다이얼로그 닫기 전에 목록 새로고침
      if (onSubmitted) {
        console.log("onSubmitted 콜백 호출");
        await onSubmitted();
      }
      ...
      toast.error(error.message || "쿠폰 저장에 실패했습니다.");
    }
  };
